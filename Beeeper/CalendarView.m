...
static const NSInteger CalendarViewYearsAround      = 2;
static const NSInteger CalendarViewYearsInLine      = 4;
...
static const NSInteger CalendarViewMaxLinesCount    = 3;
...
- (void)generateMonthRects
{
    ...
    NSArray *monthNames = [formater shortStandaloneMonthSymbols];
    NSInteger index = 0;
    CGFloat x = CalendarViewMonthTitleOffsetY;
    CGFloat y = 38;
    ...
    for (NSString *monthName in monthNames) {
        ...
        monthRect.str = [monthName uppercaseString];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (mode != CM_Years) {
        ...
        if (type == 1) {
           ...
           monthName =[monthNames[(currentMonth - 1)] uppercaseString];
        }
...
- (void)tap:(UITapGestureRecognizer *)recognizer
{
    ...
    if (CGRectContainsPoint(monthTitleRect, touchPoint)) {
        if (type != CTMonth) {
            ...
            NSLog(@"OK");
            NSDate *currentDate = [self currentDate];
            if ( _calendarDelegate && [_calendarDelegate respondsToSelector:@selector(didDoubleTapCalendar:withType:)]) {
